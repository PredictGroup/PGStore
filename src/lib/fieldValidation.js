...
* read the message.. ;)
*/
const usernamePattern = /^[a-zA-Z0-9]{6,35}$/
...
export default function fieldValidation(state, action) {
  ...
  switch (field) {
    ...
    case ('area'):
      ...
      if (!area) {
        return state.setIn(['form', 'fields', 'areaHasError'],
          false)
          .setIn(['form', 'fields', 'areaErrorMsg'], '')
      } else {
        return state.setIn(['form', 'fields', 'areaHasError'],
          true)
          .setIn(['form', 'fields', 'areaErrorMsg'],
            "Не указана зона доставки")
      }
    ...
    case ('store'):
      ...
      if (!store) {
        return state.setIn(['form', 'fields', 'storeHasError'],
          false)
          .setIn(['form', 'fields', 'storeErrorMsg'], '')
      } else {
        return state.setIn(['form', 'fields', 'storeHasError'],
          true)
          .setIn(['form', 'fields', 'storeErrorMsg'],
