...
interface Props {
  navigation: AuthStackScreenProps<'Register'>; //StackNavigationProp<AuthStackParamList, 'Register'>;
}
...
const Register: React.FC<Props> = ({navigation}) => {
  ...
  var leftMessageType = FORGOT_PASSWORD;
  ...
  const inputActionHandler = async (field: string, payload: string) => {
    ...
    await dispatch(onAuthFormFieldChange(field, payload, REGISTER));
  };
  ...
  let onButtonPress = async () => {
    await dispatch(
      signup(
        auth.form.fields.username,
        auth.form.fields.email,
        auth.form.fields.password,
      ),
    )
      .then(() => {
        inputActionHandler('username', '');
        navigation.replace('Registered');
      })
      .catch(error => {
    ...
    <SafeAreaView style={theme.container}>
      ...
      <ScrollView
        ...
        <TextInput
          ...
          value={auth.form.fields.username}
          placeholder="Enter username"
          onChangeText={text => inputActionHandler('username', text)}
          error={auth.form.fields.usernameHasError}
        ...
        <HelperText
          style={{marginLeft: 25, marginRight: 25}}
          type="error"
          visible={auth.form.fields.usernameHasError}>
          {auth.form.fields.usernameErrorMsg}
        </HelperText>
        ...
        <TextInput
          ...
          value={auth.form.fields.passwordAgain}
          secureTextEntry={secureTextEntry}
          placeholder="Enter password again"
          onChangeText={text => inputActionHandler('passwordAgain', text)}
          error={auth.form.fields.passwordAgainHasError}
          disabled={auth.form.isFetching}
        ...
        <HelperText
          style={{marginLeft: 25, marginRight: 25}}
          type="error"
          visible={auth.form.fields.passwordAgainHasError}>
          {auth.form.fields.passwordAgainErrorMsg}
        </HelperText>
